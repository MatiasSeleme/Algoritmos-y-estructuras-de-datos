def criterio_comparacion(value, criterio):
    if isinstance(value, (int, str, bool)):
        return value
    else:
        dic_atributos = value.__dict__
        if criterio in dic_atributos:
            return dic_atributos[criterio]
        else:
            print('no se puede ordenar por este criterio')


class Lista():

    def __init__(self):
        self.__elements = []

    def insert(self, value, criterio=None):
        # print('criterio de insercion', criterio)
        if len(self.__elements) == 0 or criterio_comparacion(value, criterio) >= criterio_comparacion(self.__elements[-1], criterio):
            self.__elements.append(value)
        elif criterio_comparacion(value, criterio) < criterio_comparacion(self.__elements[0], criterio):
            self.__elements.insert(0, value)
        else:
            index = 1
            while criterio_comparacion(value, criterio) >= criterio_comparacion(self.__elements[index], criterio):
                index += 1
            self.__elements.insert(index, value)

    def search(self, search_value, criterio=None):
        position = None
        first = 0
        last = self.size() - 1
        while (first <= last and position == None):
            middle = (first + last) // 2
            if search_value == criterio_comparacion(self.__elements[middle], criterio):
                position = middle
            elif search_value > criterio_comparacion(self.__elements[middle], criterio):
                first = middle + 1
            else:
                last = middle - 1
        return position

    def search_r(self, search_value, first, last, criterio=None):
        middle = (first + last) // 2
        if first > last:
            return None
        elif search_value == criterio_comparacion(self.__elements[middle], criterio):
            return middle
        elif search_value > criterio_comparacion(self.__elements[middle], criterio):
            return self.search_r(search_value, middle+1, last, criterio)
        else:
            return self.search_r(search_value, first, middle-1, criterio)
 
    def delete(self, value, criterio=None):
        return_value = None
        pos = self.search(value, criterio)
        if pos is not None:
            return_value = self.__elements.pop(pos)
        return return_value

    def size(self):
        return len(self.__elements)

    def barrido(self):
        for value in self.__elements:
            print(value)

    def order_by(self, criterio=None, reverse=False):
        dic_atributos = self.__elements[0].__dict__
        if criterio in dic_atributos:
            def func_criterio(valor):
                return valor.__dict__[criterio]

            self.__elements.sort(key=func_criterio, reverse=reverse)
        else:
            print('no se puede ordenar por este criterio')

    def get_element_by_index(self, index):
        return_value = None
        if index >= 0 and index < self.size():
            return_value = self.__elements[index]
        return return_value

    def set_value(self, value, new_value, criterio=None):
        pos = self.search(value, criterio)
        if pos is not None:
            value = self.delete(value)
            self.insert(new_value, criterio)

class Superheroe():
    def __init__(self, name, year, house, traje):
        self.name = name
        self.year = year
        self.house = house
        self.traje = traje

    def __str__(self):
        return f'{self.name} - {self.year} - {self.house} - {self.traje}'
    
superheroes = Lista()

s1 = Superheroe("Capitana Marvel", 1968, "Marvel", "usa traje")
s2 = Superheroe("Mujer Maravilla", 1941, "DC", "usa armadura")
s3 = Superheroe("Linterna Verde", 1940, "DC", "usa traje")
s4 = Superheroe("Wolverine", 1974, "Marvel", "usa traje")
s5 = Superheroe("Dr. Strange", 1963, "DC", "usa traje")
s6 = Superheroe("Flash", 1930, "DC", "usa traje")
s7 = Superheroe("Star-Lord", 1976, "Marvel", "usa armadura")

superheroes_valores= [s1, s2, s3, s4, s5, s6, s7]

for superheroe in superheroes_valores:
    superheroes.insert(superheroe, "name")

#insert (lista, criterio por el que se va a llamar el nodo: en este caso el nombre)
print(superheroes.barrido())

superheroes.delete("Linterna Verde", "name")

print(superheroes.barrido())

for i in superheroes_valores:
    if i.name == "Wolverine":
        print("\nel año de aparicion de wolverine es: ", i.year)

for i in superheroes_valores:
    if i.name == "Dr. Strange":
        i.house = "Marvel"

print(superheroes.barrido())

print("Superheroes de traje:")
print("\n")

for i in superheroes_valores:
    if "traje" in i.traje:
        print(i)


print("Superheroes con armadura:")
print("\n")

for i in superheroes_valores:
    if "armadura" in i.traje:
        print(i)

print("Superheroes con fecha de aparicion en 1963:")
print("\n")
for i in superheroes_valores:
    if i.year == 1963:
        print(i.name, i.house)


print("Capitana Marvel y Mujer maravilla casa:")
print("\n")

for i in superheroes_valores:
    if i.name == "Capitana Marvel":
        print(i.name, i.house)
    if i.name == "Mujer Maravilla":
        print(i.name, i.house)


print("info Flash, Starlord:")
print("\n")

for i in superheroes_valores:
    if i.name == "Flash":
        print(i)

for i in superheroes_valores:
    if i.name == "Star-Lord":
        print(i)

print("Lista Nueva con iniciales B, M, S")
print("\n")

BMS = Lista()

letra = ["B", "M", "S"]

SuperBMS = []

for i in superheroes_valores:
    if i.name[0] in letra:
        SuperBMS.append(i)

for i in SuperBMS:
    BMS.insert(i, "name")

print(BMS.barrido())

M = 0
DC = 0

for i in superheroes_valores:
    if i.house == "DC":
        DC += 1
    if i.house == "Marvel":
        M += 1


print("Cantidad de superheroes de DC:", DC)
print("\n")

print("Cantidad de superheroes de Marvel:", M)
print("\n")