...
for i in maravillas:
    valor = mi_grafo.search_vertice(i.name)
    posicion1 = mi_grafo.get_element_by_index(valor)
    for j in maravillas:
        valor = mi_grafo.search_vertice(j.name)
        posicion2 = mi_grafo.get_element_by_index(valor)
        checker = mi_grafo.is_adyacent(posicion1[0], posicion2[0])
        if posicion1 != posicion2 and checker == False:
            value = randint(100, 5000)
            mi_grafo.insert_arist(posicion1[0], posicion2[0], value)
...
for i in maravillas_naturales:
    ...
    for j in maravillas:
        valor = mi_grafo.search_vertice(j.name)
        posicion2 = mi_grafo.get_element_by_index(valor)
        index_A = dic[posicion1[0]]
        index_B = dic[posicion2[0]]
        if (
            maravillas[index_B].country[0]
            in maravillas_naturales[index_A].country
        ):
            if (maravillas[index_B].country in countries) == False:
                countries.append(maravillas[index_B].country)
print()
for i in countries:
    print(f"{i[0]} tiene los 2 tipos de maravilla")
print()
...
for i in maravillas:
    ...
    for j in maravillas:
        ...
        index_A = dic[posicion1[0]]
        index_B = dic[posicion2[0]]
        for k in maravillas[index_B].country:
            for l in maravillas[index_A].country:
                if k == l and posicion2[0] != posicion1[0]:
                    if (k in no_natural) == False:
                        no_natural.append(k)
print()
for i in maravillas_naturales:
    valor = mi_grafo.search_vertice(i.name)
    ...
    for j in maravillas_naturales:
        ...
        for k in maravillas_naturales[index_B].country:
            for l in maravillas_naturales[index_A].country:
                if k == l and posicion2[0] != posicion1[0]:
                    if (k in natural) == False:
                        natural.append(k)
print()

for i in natural:
    print(f"{i} mas de una maravillas natural")
print()
for i in no_natural:
    print(f"{i} mas de una maravillas arquitectura")
print()

bosque = mi_grafo.kruskal()
print("Arbol de expansion minimo para cada maravilla")
for arbol in bosque:
    print("arbol")
